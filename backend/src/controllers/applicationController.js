...
export async function apply(req, res) {
  try {
    ...
    if (!challenge)
      return res.status(404).json({ message: "Published challenge not found" });
    ...
    if (error.code === 11000)
      return res
        .status(409)
        .json({ message: "You have already applied to this challenge" });
...
export async function myApplications(req, res) {
  try {
    const applications = await Application.find({ startupId: req.user._id })
...
export async function runEligibility(req, res) {
  try {
    ...
    if (
      ...
      return res.status(403).json({ message: "Access denied" });
    application.eligibility = checkEligibility(
      application.startupId,
      application.challengeId,
    );
    application.status = application.eligibility.eligible
      ? "eligible"
      : "rejected";
