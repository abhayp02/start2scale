...
export async function listMilestones(req, res) {
  try {
    return res.json({
      milestones: await Milestone.find({ pilotId: req.params.pilotId }).sort({
        dueDate: 1,
      }),
    });
  } catch {
...
export async function updateMilestone(req, res) {
  try {
    ...
    if (
      ...
      !["government", "evaluator"].includes(req.user.role)
    )
      return res
        .status(403)
        .json({
          message: "Only government or evaluator users can verify milestones",
        });
    if (req.body.status) milestone.status = req.body.status;
    if (req.body.status === "verified") {
      milestone.paymentDue = true;
