...
export async function listTemplates(req, res) {
  try {
    return res.json({ templates: await Template.find().sort({ type: 1 }) });
  } catch {
...
export async function generateContract(req, res) {
  try {
    ...
    if (!["pilot-agreement", "ip-data-clause"].includes(type))
      return res
        .status(400)
        .json({
          message:
            "Only pilot-agreement and ip-data-clause templates can be AI-filled",
        });
